...
def ensure_dataframe(
    ...
    *,
    ...
    if isinstance(data, pd.DataFrame):
        ...
        # Curated pandas-boundary suppression: ``data`` is an arbitrary mapping /
        # sequence coercible to a DataFrame; pandas-stubs has no overload for the
        # broad ``object`` here, so we accept the coercion at the boundary.
