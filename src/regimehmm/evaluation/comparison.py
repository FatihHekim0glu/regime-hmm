...
from regimehmm._typing import ReturnsLike
...
def block_bootstrap_sharpe_gap(
    ...
    *,
    ...
    (:func:`regimehmm._rng.make_rng`), so the CI is byte-identical for a fixed ``seed``.
    ...
    for b in range(n_bootstrap):
        ...
        idx[0] = pos
        # Wrap in ``np.asarray`` so the variable type is a concrete ndarray under
        # strict mypy (numpy's ``size=`` overloads are ambiguous for a plain int).
        restarts = np.asarray(rng.random(size=t), dtype="float64")
